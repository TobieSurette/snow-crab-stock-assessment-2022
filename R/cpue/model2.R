...
parameters <- list(alpha_zone = c(5.1192791085, 6.2948735164, 4.9486955406, 5.2387046350),
                   ...
                   beta_day_1 = c(0,0,0,0),
                   beta_day_2 = c(1,1,1,1),
                   beta_setting = 0.0920456312,
                   log_scale_soak_time = log(c(3, 3, 3, 3)),
                   log_rate_soak_time = log(c(0.3, 0.3, 0.3, 0.3)),
                   gamma = 0,                           # Johnson's SU distribution skewness parameter.
                   log_delta = 0,                       # Johnson's SU distribution shape parameter.
                   log_lambda = 0)                      # Johnson's SU distribution scale parameter.
...
t <- seq(-3, 3, len = 10000)
plot(t, djohnson(t, gamma = obj$par["gamma"], lambda = exp(obj$par["log_lambda"]), delta = exp(obj$par["log_delta"])), type = "l")

theta <- obj$par[grep("alpha_zone", names(obj$par))]
names(theta) <- zones
gbarplot(theta)


beta0 <- obj$par[grep("beta_day_0", names(obj$par))]
names(beta0) <- zones
beta1 <- obj$par[grep("beta_day_1", names(obj$par))]
names(beta1) <- zones
beta2 <- obj$par[grep("beta_day_2", names(obj$par))]
names(beta2) <- zones
t <- seq(0, 60, len = 10000)
plot(t, exp(beta0[1] * t + beta1[1] * (1-exp(-beta2[1]*t))), cex = 0.2, ylim = c(0, 1), ylab = "Scaling")
grid()
for (i in 2:4){
   points(t, exp(beta0[i] * t + beta1[i] * (1-exp(-beta2[i]*t))), cex = 0.2)
}
