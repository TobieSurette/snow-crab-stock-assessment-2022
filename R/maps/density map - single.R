...
for (i in 1:length(m$variables)){
   ...
   file <- paste0(tolower(category(m$variables[i], language = language, symbols = FALSE)), " - ", year)
   file <- paste0(path,  file, " - ", language)
   if (output == "pdf"){
      pdf(file = paste0(file, ".pdf"), height = 7.5, width = 8.5)
   }
   if (output == "tiff"){
      tiff(file = paste0(file, ".tiff"), compression = "lzw", units = "in", res = 300, height = 7.5, width = 8.5)
   }
