...
setwd("/Users/crustacean/Desktop/gulf.photos/scs.photo.2021")
files <- dir(pattern = "^GP")

files <- files[-grep("PORTE", files)]

# Load tow data:
x <- read.scsset(year = year, valid = 1, survey = "regular")
...
thin <- 10 # Image thinning factor (large numbers = more thinning, 1 = no thinning)
...
if (pdf){
   pdf(file = paste0("/Users/crustacean/Desktop/snow-crab-stock-assessment-2021/scs photo map ", year, ".pdf"), width = 13, height = 11)
}else{
   quartz()
}
...
for (i in 1:length(mif)){
   ...
   if (length(j) == 1){
      ...
      if (length(file) == 1){
         ...
         col <- c("blue", "red", "green", "yellow", "grey")[match(x$zone[j], c("12", "12E", "12F", "19", "BUFFER"))]
...
legend("bottomleft",
       ...
       pch = 22, col = c("blue", "red", "green", "yellow", "grey"), lwd = NA, pt.cex = 3, pt.lwd = 3,
       bg = "grey90", cex = 1.25)
