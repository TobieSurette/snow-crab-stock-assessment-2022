...
BMMGE95.2021.mu    <- 80949.8  # Kriged commercial biomass.
BMMGE95.2021.sigma <- 5591.8   # Standard error of kriged commercial biomass.

# Calculate exploitation rate and TAC:
ER  <- harvest.control.rule(BMMGE95.2021.mu, species = 2526)
TAC <- total.allowable.catch(BMMGE95.2021.mu, species = 2526)  # Total allowable catch for sGSL.
...
BREC.2022.mu       <- 73120    # Projected R-1 recruitment from the Bayesian projection model.
BREC.2022.sigma    <- 14440    # Standard error or projected R-1 recruitment.
...
data <- data.frame(year = 1997:2021,
                   landings = c(17.66, 13.86, 15.52, 19.18, 18.51, 26.18, 21.16, 31.66, 36.08, 29.12, 26.87, 24.46, 23.64, 9.549,
                                10.71, 21.96, 26.05, 24.44, 25.91, 21.71, 43.656, 24.260, 31.707, 28.156, 24.480),
                   ...
                   MMGE95.mu = c(64.5184, 64.5184, 57.8125, 56.7565, 50.621, 60.3283, 79.2275, 84.4475, 103.1457, 82.5652, 73.6453, 66.3714,
                                 52.9209, 31.0153, 35.9294, 62.8407, 74.7775, 66.709, 67.9896, 58.9269, 98.3942, 65.7376, 80.746, 79.06550, 77.74807),
                   MMGE95.sigma = c(5.6785, 5.6785, 6.6617, 4.9687, 4.8133, 5.7457, 6.0774, 5.8931, 5.699, 4.8234, 4.2417, 3.3922, 3.0653,
                                    1.8656, 2.0665, 3.6529, 5.3264, 6.8484, 4.3836, 4.0608, 6.0042, 4.578, 5.302936, 5.364855, 5.397385))
...
BMMGE95.2021 <- rlnorm(n, (1 - bias.scs) * BMMGE95.2021.mu, (1 - bias.scs) * BMMGE95.2021.sigma) # Simulate biomass for current year.
BREC.2022    <- rlnorm(n, (1 - bias.rec) * BREC.2022.mu, (1 - bias.rec) * BREC.2022.sigma)       # Projected R-1 mu and sigma.
...
BREM.2022 <- repvec(BMMGE95.2021 * S, ncol = length(catch)) - repvec(catch, nrow = n)

# Projected probability of remaining biomass being below Blim:
Plim <- apply(BREM.2022 < Blim, 2, function(x) sum(x) / length(x))

# Projected probability of total biomass being below Busr:
Pusr <- apply((repvec(BREC.2022, ncol = length(catch)) + BREM.2022) < Busr, 2, function(x) sum(x) / length(x))

# Probability of exceeding ER in 2021:
ER.2022 <- repvec(catch, nrow = n) / repvec(BMMGE95.2021, ncol = length(catch))  # Calculate simulated exploitation rates:
PER <- apply(ER.2022 > ER, 2, function(x) sum(x) / length(x))
...
tab <- data.frame(catch = catch,
                  ...
                  P.usr = Pusr,
                  P.flim = PER,
                  mu    = apply(repvec(BREC.2022, ncol = length(catch)) + BREM.2022, 2, mean),
                  lci   = apply(repvec(BREC.2022, ncol = length(catch)) + BREM.2022, 2, quantile, p = 0.025),
                  uci   = apply(repvec(BREC.2022, ncol = length(catch)) + BREM.2022, 2, quantile, p = 0.975))
