...
# Number of original random stations from 2013 to substitute:
n <- 100  
...
# Assign tow IDs to survey grids using previous year:
...
ix <- which(unlist(lapply(mif, function(x) x$tow.id)) == "") # Grids with no assigned station.
s <- which(!(substr(stations$tow.id, 1, 5) %in% substr(unlist(lapply(mif, function(x) x$tow.id)), 1, 5))) # 2020 stations that are not assigned to a grid.
...
ix <- which(!((1:length(mif)) %in% as.numeric(substr(unlist(lapply(mif, function(x) x$tow.id)), 3, 5))))  # Tow IDs not in MIF grids.
...
# Assign MIF grids to 2013 stations:
y$grid <- NA
for (i in 1:length(mif)){
   tmp <- km2deg(mif[[i]]$x, mif[[i]]$y)
   mif[[i]]$longitude <- tmp$longitude
   mif[[i]]$latitude  <- tmp$latitude
   ix <- which(in.polygon(as.polygon(mif[[i]]$longitude, mif[[i]]$latitude), lon(y), lat(y)))
   if (length(ix) == 1) y$grid[ix] <- i
}

# Assign MIF grids to 2020 stations:
stations$grid <- NA
for (i in 1:length(mif)){
   tmp <- km2deg(mif[[i]]$x, mif[[i]]$y)
   mif[[i]]$longitude <- tmp$longitude
   mif[[i]]$latitude  <- tmp$latitude
   ix <- which(in.polygon(as.polygon(mif[[i]]$longitude, mif[[i]]$latitude), lon(stations), lat(stations)))
   if (length(ix) == 1) stations$grid[ix] <- i
}

# Identify stations which have moved since 2013:
tab$moved <- FALSE
for (i in 1:nrow(tab)){
   d <- distance(tab$longitude[i], tab$latitude[i], lon(y), lat(y))[1, ]
   if (min(d) > 1.5) tab$moved[i] <- TRUE
}
...
# Assign MIF grids to table:
tab$grid <- NA
for (i in 1:length(mif)){
   tmp <- km2deg(mif[[i]]$x, mif[[i]]$y)
   mif[[i]]$longitude <- tmp$longitude
   mif[[i]]$latitude  <- tmp$latitude
   ix <- which(in.polygon(as.polygon(mif[[i]]$longitude, mif[[i]]$latitude), tab$longitude, tab$latitude))
   if (length(ix) == 1) tab$grid[ix] <- i
}

# Select 20 stations that will test the effect of having moved the stations:
flag <- FALSE
while (!flag){
   print(123)
   ix <- sort(sample(which(tab$moved & tab$new.station == "No" & tab$station == "regular"), 20))
   iy <- match(tab$grid[ix], y$grid)
   if (!any(is.na(iy))) flag <- TRUE
}
tab$longitude.2013 <- NA
tab$latitude.2013 <- NA
tab$longitude.2013[ix] <- lon(y)[iy]
tab$latitude.2013[ix] <- lat(y)[iy]
...
# Write table:
file <- "C:/Users/SuretteTJ/Desktop/github/snow-crab-stock-assessment-2022/results/tables/scs.stations.2022.csv"
write.table(tab[setdiff(names(tab), c("moved", "grid"))], file = file, row.names = FALSE, col.names = TRUE, sep = ",")
